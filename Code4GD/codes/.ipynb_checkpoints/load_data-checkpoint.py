...
class Dataset_Rating(Dataset):
    ...
    def __init__(self, dataset, task_dir):
        ...
        # userID to index
        with open(os.path.join(task_dir, 'user2id.txt')) as f:
            self.userID = {}
            for line in f.readlines():
                userID, index = line.strip().split()
                self.userID[userID] = int(index)
        # itemID to index
        with open(os.path.join(task_dir, 'item2id.txt')) as f:
            self.itemID = {}
            for line in f.readlines():
                itemID, index = line.strip().split()
                self.itemID[itemID] = int(index)
        
    def __len__(self):
        return len(self.dataset)
    
    def __getitem__(self, index):
        return self.userID[self.dataset[index][0]], self.itemID[self.dataset[index][1]], int(self.dataset[index][2])-1
    ...
    def load_dataset(self):
        ...
        with open(os.path.join(self.task_dir, 'RatingDataset.txt')) as f:
            for line in f.readlines():
                ...
                if user not in user2rating:
                    user2rating[user] = []
                user2rating[user].append(rating)
