...
class BaseModel(object):
    ...
    def train_batch(self,):
        ...
        for i in range(n_batch):
            ...
            scores = self.model(triple[:,0], triple[:,1]) # 获取分数

            pos_scores = scores[[torch.arange(len(scores)).cuda(),torch.LongTensor(triple[:,2]).cuda()]] # 正例分数
            max_n = torch.max(scores, 1, keepdim=True)[0]
            loss = torch.sum(- pos_scores + max_n + torch.log(torch.sum(torch.exp(scores - max_n),1)))   ## Loss函数,公式(6)--> 交叉熵损失+数值稳定性
